...
$.ajax({
  ...
  data: {
    key: 'WHTBZ-O35RI-NBZGF-U6W4H-LLQGS-F4FFX',  // 这里要写你的KEY!!!
    output: 'jsonp',
  },
  dataType: 'jsonp',
...
function showWelcome() {
  ...
  let dist = getDistance(120.077516, 30.137547, ipLoacation.result.location.lng, ipLoacation.result.location.lat); //这里换成自己的经纬度
...
function createtime1() {
  ...
  var ascll = [
    `welcome to linear-zh (๑•̀ㅂ•́)و✧ 's blog `,
    `let's go !!! 🍭🍭🍭`,
    ` 
...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by linear-zh %c 你正在访问 linear-zh (๑•̀ㅂ•́)و✧ 's blog",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| linear-zh (๑•̀ㅂ•́)و✧ ") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('linear-zh (๑•̀ㅂ•́)و✧ 的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
...
document.addEventListener('visibilitychange', function () {
  if (document.hidden) {
    //离开当前页面时标签显示内容
    document.title = ' o(≧▽≦)ツ┏━┓ 看什么去了~ ';
    clearTimeout(titleTime);
  } else {
    //返回当前页面时标签显示内容
    document.title = ' (✿◡‿◡) 还知道回来~ ';
    //两秒后变回正常标题
